...
`public/googleb670262adc5e88c9.html` is the Google Search Console ownership-verification
file for the URL-prefix property `https://szlab-ai.github.io/porthole-site/`. **Do not
delete it** — removing it revokes verification. The sitemap is generated by
`@astrojs/sitemap` at `/sitemap-index.xml` and is submitted through Search Console
(a `robots.txt` at this project-page path is never read by crawlers, which only fetch
`szlab-ai.github.io/robots.txt`).
